"""
inference.py  –  TFLite inference helper
-----------------------------------------
Standalone module for running emotion detection locally (no server required).
Also used internally by app.py.
...
class EmotionClassifier:
    ...
    def predict(self, face_bgr: np.ndarray) -> dict:
        ...
        ----------
        face_bgr : np.ndarray
            Face crop in BGR format (any size; will be resized internally).

        Returns
        -------
        dict with keys: emotion (str), confidence (float), scores (dict)
        """
